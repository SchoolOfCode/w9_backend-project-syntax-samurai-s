...
export const snippets = [
    ...
    {
        title : "For Loop",
        description : "Use a for loop run the same code over and over again, each time with a different value.",
...

    {
        ...
        codesyntax : [ "while (condition) { // code block to be executed}" ],
...

    {
        ...
        codesytax : [ "const scrollTop = () => window.scrollTo(0, 0); scrollTop()" ],
...

{
            ...
            "codesyntax" : [ "function positiveSum(arr) { ","// code block",
            "var arraySum = 0;    //create & set variable arraySum to 0",
            "for (i = 0; i < arr.length; i++) {    //loop through array",
          "  if (arr[i] > 0) {",
          "arraySum += arr[i];    //if positive integer add to arraySum }",
            "} ",
            "} return arraySum;",
        "}",
        "positiveSum();" ],
...

{
          ...
          "var arraySum = 0;    //create & set variable arraySum to 0;",
"          if (array.length === 1 && array[0] == 0) { //if array length is 1 and number is 0 return even",
               "return 'even';",
            "if (array[0] == 1) {    //nested if; number is 1 return odd",
                " return 'odd';",
"}",
           "} else if (array.length === 0) {    //if nothing in array return even",
            "return 'even';",
            "} else if (array.length > 0) {",
               "for (i = 0; i < array.length; i++) {",
               "arraySum += array[i];    //loop through array and sum total to arraySum",
           "}",
           "if ((arraySum % 2) === 0) {",
                "return 'even';    //return even if arraySum is divisible by 2",
                "} else {",
              "return \"odd\";    //return odd if arraySum is not divisible by 2",
              "}",
       " }",
       " }",
